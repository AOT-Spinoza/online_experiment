...
export function preloadConfig({ videos, message, phase, blockIndex = null }) {
  return {
    type: PreloadPlugin,
    video: videos,
    ...
    on_error: (url) => {
      ...
      console.error(
        `[preload] failed to load video (phase=${phase}` +
      ...
      console.error(
        ...
        `  - Video host returning 4xx/5xx — check the URL above directly in a browser tab\n` +
